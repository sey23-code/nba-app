...
games["is_home"] = games["MATCHUP"].apply(lambda x: "Y" if isinstance(x, str) and "vs." in x else "N")
...
#print(lakers[["GAME_DATE", "PTS", "avg_pm_last_15", "WL"]])
...
home_games = games[games["is_home"] == "Y"]
away_games = games[games["is_home"] == "N"]
...
merged_games = merged_games.drop(columns=["WL_HOME", "WL_AWAY"])
merged_games = merged_games.dropna()
pd.set_option('display.max_columns', None)
pd.set_option('display.width', None)
pd.set_option('display.max_rows', 100)
print(merged_games.head(30))
